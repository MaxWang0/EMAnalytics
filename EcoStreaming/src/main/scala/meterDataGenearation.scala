...
object meterDataGenearation extends App {
  ...
  //var usage_kwh = 0.0
  // infinite loop
  var inFile = Source.fromFile("/home/ecomotto/Downloads/Data/IntervalMeterUsage.csv").getLines().drop(1).map(_.split(","))
  //  while(true) {
  ...
  inFile.foreach { k =>
    ...
    val usage_start_time = k(2)
    val usage_end_time = k(3)
    ...
    if (iDay > 5) {
      usage_kwh = k(4).toDouble
    } else {
      usage_kwh = k(4).toDouble
    }

    val estimated_actual = k(5)
    val consumption_generation = k(6)
